...
export const filterPrice = (minprice,maxprice)=> async (dispatch)=>{
    ...
    if(minprice && maxprice){
        url = `filter/price/${minprice}/${maxprice}/`
    }else if(minprice && maxprice === ''){
        url = `filter/price/min/${minprice}/`
    }else{
        url = `filter/price/max/${maxprice}/`
    }
...
export const filterRating = (rating)=> async (dispatch)=>{
    ...
    await axios.get(`${process.env.REACT_APP_API_URL}filter/rating/${rating}/`,config).then((res)=>{
        dispatch({
            type: FILTER_RATING,
            payload: res.data
        })
...
export const filterCondition = (condition)=> async (dispatch)=>{
    ...
    await axios.get(`${process.env.REACT_APP_API_URL}filter/condition/${condition}/`,config).then((res)=>{
        dispatch({
